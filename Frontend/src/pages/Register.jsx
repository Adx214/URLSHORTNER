...
const Register = ({logged,setlogged}) => {
  ...
  const submitform = async(data) => {
    ...
    try {
      let res = await axios.post('http://localhost:5050/register', {
        name: data.name,
        userid: data.userid,
      ...
      if (res.status === 201) {
        ...
        setCustomError(res.data.message || 'Registration failed');
      }
    } catch (error) {
      setCustomError('Network or server error');
    }
  }
  ...
  return (
    <div>
      <h1 className='text-5xl text-black underline text-center'>REGISTER FORM</h1>
      ...
      <form className="max-w-sm mx-auto mt-10" onSubmit={handleSubmit(submitform)}>
        ...
        <div className="mb-5">
          <label htmlFor="email" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your UserId</label>
          <input
            type="text"
            id="email"
            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
